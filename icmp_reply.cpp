...
struct icmp_header
{
	uint8_t		type;					// (8Bit)		ICMP Type
	uint8_t		code;					// (8Bit)		ICMP Code
	uint16_t	checksum;				// (16Bit)		ICMP Header Checksum
	uint16_t	id;						// (16Bit)		Identifier(BE + LE)
	uint16_t	seq;					// (16Bit)		Sequence Number(BE + LE)
	//uint8_t*	data;					// (Pointer)	ICMP Pointer
};
#pragma pack(pop)
...
void icmp_checksum(_Inout_ struct icmp_header* _pIp, _In_ int _header_len);
...
int main(void)
{
...
#ifdef _TEST_CASE_6		// Virtual Machine 3
	_info.a_host = "a8:5e:45:55:1e:8a";
	_info.a_ip = "192.168.42.2";
	_info.v_host = "d4:be:d9:92:38:1f";
	_info.v_ip = "192.168.42.5";
	_info.g_host = "88:36:6c:7a:56:40";
	_info.g_ip = "192.168.42.1";
#endif
	...
	for (;;)
	{
		...
		if ((data = (uint8_t*)pcap_next(_info._dev_handle, &header)) != NULL)
		{
			...
			if (pIp->protocol == PROTOCOL_ICMP)
			{
				struct icmp_header* pIcmp;
				pIcmp = (struct icmp_header*)(data + dataPointer);
				dataPointer += sizeof(*pIcmp);

				uint8_t* icmp_data = (data + dataPointer);
				...
				if (pIcmp->type == 0x08)
				{
					...
					pIcmp->type = 0x00;
					int icmp_size = ntohs(pIp->total_len) - sizeof(*pIp);
					icmp_checksum(pIcmp, icmp_size);
					memcpy(test + dataPointer, pIcmp, sizeof(*pIcmp));
					dataPointer += (sizeof(*pIcmp));

					strcat((char*)(test + dataPointer), (char*)icmp_data);

					// Backwarding
					pcap_sendpacket(_info._dev_handle, test, header.len);
					is_forward = false;
				}
			}
			if (is_forward)
...
void icmp_checksum(_Inout_ struct icmp_header* _pIcmp, _In_ int _header_len)
{
	uint16_t* pIps = (uint16_t*)_pIcmp;
	uint16_t len = _header_len, checksum;
	uint32_t check = 0;
